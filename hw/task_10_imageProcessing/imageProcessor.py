...
class ImageProcessingApp:
    ...
    def setup_gui(self):
        ...
        # 1. Resolution & Zoom
        tk.Label(control_frame, text="Resolution:").pack(anchor="w", padx=5)
        ...
        tk.Label(control_frame, text="Shape Noise (0-1):").pack(anchor="w", padx=5, pady=(5,0))
        ...
        # SPLIT BUTTONS HERE
        tk.Button(control_frame, text="1. Generate New Scene", command=self.generate_new_scene, bg="#dddddd", height=2).pack(fill=tk.X, padx=5, pady=2)
        tk.Button(control_frame, text="2. Apply Noise to Scene", command=self.apply_noise_to_current, bg="#e0e0e0", height=2).pack(fill=tk.X, padx=5, pady=2)
        ...
        tk.Button(control_frame, text="Process (Solve)", command=self.solve_image, bg="#add8e6", height=2).pack(fill=tk.X, padx=5, pady=2)
        tk.Button(control_frame, text="Save All Images", command=self.save_images).pack(fill=tk.X, padx=5, pady=10)

        # Display
        self.panel_left = tk.Label(self.display_frame, text="[Generated Image Preview]", bg="#f0f0f0")
    ...
    def apply_noise_to_current(self):
        """Re-draws the scene using stored metadata and CURRENT noise sliders."""
        if not self.scene_data:
            messagebox.showwarning("No Scene", "Please generate a scene first!")
            return
    ...
    def solve_image(self):
        ...
        if self.noise_color_type.get() == "Salt & Pepper" and self.noise_color_amt.get() > 0.2:
    ...
    def save_images(self):
        if self.img_noisy is None: return
        base_path = filedialog.asksaveasfilename(defaultextension=".jpg", filetypes=[("JPEG", "*.jpg"), ("PNG", "*.png")])
        if base_path:
            cv2.imwrite(base_path, self.img_noisy)
            clean_path = base_path.replace(".", "_ground_truth.")
            cv2.imwrite(clean_path, self.img_clean)
            if self.img_processed is not None:
                proc_path = base_path.replace(".", "_solved.")
                cv2.imwrite(proc_path, self.img_processed)
