...
# -*- coding: utf-8 -*-
"""
Maze Homework GUI - Part I + Part II (v4)
...
Run:
  python3 maze_homework_part12_gui_v4.py
"""
...
from PIL import Image, ImageTk, ImageDraw, ImageOps, ImageOps
...
class MazeHomeworkApp(tk.Tk):
    ...
    def on_run_part1(self):
        ...
        try:
            ...
            best_exit = min(exits, key=lambda e: abs(e[0]-sx)+abs(e[1]-sy))
            t0 = time.perf_counter()
            path, visited = astar_on_mask(free, (sx, sy), best_exit, allow_diag=False)
            ...
            self.lbl_right_status.config(text=f"Part I: {'FOUND' if path else 'NOT FOUND'} | visited={len(visited)} time={dt:.4f}s")
        except Exception as e:
