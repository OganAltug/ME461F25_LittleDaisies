...
# --- GUI SETUP ---
fig, ax = plt.subplots(figsize=(12, 8)) # Wider window
# Shift graph to right to make room for stats on the left
...
# --- SIDEBAR STATISTICS (Left Side) ---
# We use fig.text to place text outside the axes
# CH1 Stats (Green)
stats_ch1 = fig.text(0.02, 0.75, "CH1\nWait...", fontsize=11, 
                     family='monospace', color='#00ff00', 
                     bbox=dict(facecolor='#111111', alpha=0.5, edgecolor='#00ff00'))

# CH2 Stats (Cyan)
stats_ch2 = fig.text(0.02, 0.50, "CH2\nWait...", fontsize=11, 
                     family='monospace', color='#00ffff',
                     bbox=dict(facecolor='#111111', alpha=0.5, edgecolor='#00ffff'))

# CH3 Stats (Red)
stats_ch3 = fig.text(0.02, 0.25, "CH3\nWait...", fontsize=11, 
                     family='monospace', color='#ff3333',
                     bbox=dict(facecolor='#111111', alpha=0.5, edgecolor='#ff3333'))
    ...
    try:
        if ser.in_waiting > 0:
            ...
            if decoded_line.startswith("METRICS:"):
                ...
                # --- PARSE METRICS FOR 3 CHANNELS ---
                # Format: M:vpp,f,p,a,d;vpp,f,p,a,d;vpp,f,p,a,d
                ...
                if len(channels_metrics) == 3:
                    ...
                    stats_ch1.set_text(f"CH1 STATS\n{'-'*10}\n" + format_stats(m1))
                    
                    # CH2
                    m2 = [float(x) for x in channels_metrics[1].split(',')]
                    stats_ch2.set_text(f"CH2 STATS\n{'-'*10}\n" + format_stats(m2))
                    
                    # CH3
                    m3 = [float(x) for x in channels_metrics[2].split(',')]
                    stats_ch3.set_text(f"CH3 STATS\n{'-'*10}\n" + format_stats(m3))
