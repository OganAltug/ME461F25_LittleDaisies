...
# --- STATUS TEXT ---
status_text = fig.text(0.5, 0.96, "WAITING", fontsize=16,
                       color='yellow', ha='center', va='top', weight='bold')
...
# --- REAL-TIME PWM LABELS (TOP OF PLOT) ---
pwm_label_ch1 = ax_main.text(0.01, 1.02, "CH1 PWM: ---- Hz | -- %",
                             transform=ax_main.transAxes, color='#00ff00',
                             fontsize=9, ha='left', va='bottom')
pwm_label_ch2 = ax_main.text(0.35, 1.02, "CH2 PWM: ---- Hz | -- %",
                             transform=ax_main.transAxes, color='#00ffff',
                             fontsize=9, ha='left', va='bottom')
pwm_label_ch3 = ax_main.text(0.69, 1.02, "CH3 PWM: ---- Hz | -- %",
                             transform=ax_main.transAxes, color='#ff6666',
                             fontsize=9, ha='left', va='bottom')
...
# --- XY CONTROLS ---
rect_ctrl = Rectangle((0.72, 0.15), 0.25, 0.28,
...
radio_x = RadioButtons(ax_radio_x, ('Channel 1', 'Channel 2', 'Channel 3'),
                       activecolor='black')

ax_radio_y = fig.add_axes([0.86, 0.20, 0.15, 0.15], facecolor='white')
radio_y = RadioButtons(ax_radio_y, ('Channel 1', 'Channel 2', 'Channel 3'),
                       activecolor='black')
...
# CH1 Stats
rect1 = Rectangle((box_left, 0.72), box_width, box_height,
                  transform=fig.transFigure,
...
text_ch1 = fig.text(box_left + 0.01, 0.73, "Waiting...",
                    transform=fig.transFigure,
                    color='black', family='monospace', fontsize=8,
                    verticalalignment='bottom')

# CH2 Stats
rect2 = Rectangle((box_left, 0.47), box_width, box_height,
                  transform=fig.transFigure,
...
text_ch2 = fig.text(box_left + 0.01, 0.48, "Waiting...",
                    transform=fig.transFigure,
                    color='black', family='monospace', fontsize=8,
                    verticalalignment='bottom')

# CH3 Stats
rect3 = Rectangle((box_left, 0.22), box_width, box_height,
                  transform=fig.transFigure,
...
text_ch3 = fig.text(box_left + 0.01, 0.23, "Waiting...",
                    transform=fig.transFigure,
                    color='black', family='monospace', fontsize=8,
                    verticalalignment='bottom')
...
def send_command(cmd):
    try:
        ser.write(cmd.encode())  # single-char commands: R, S, H
    except:
        pass
...
# --- PWM CONTROL PANELS (3 CHANNELS) ---
def send_pwm_cmd(cmd):
    try:
        ser.write((cmd + "\n").encode())
    except:
        pass
...
# CH1 controls
fig.text(0.04, 0.17, "PWM CH1", color='white', fontsize=12, weight='bold')

ax_freq1 = plt.axes([0.04, 0.10, 0.12, 0.07], facecolor='white')
freq1 = RadioButtons(ax_freq1, ('500','1000','5000','10000','20000'),
                     activecolor='black')
for lbl in freq1.labels: lbl.set_color('black')

def freq1_changed(label):
    send_pwm_cmd(f"F1_{label}")
freq1.on_clicked(freq1_changed)

ax_duty1 = plt.axes([0.17, 0.10, 0.12, 0.07], facecolor='white')
duty1 = RadioButtons(ax_duty1, ('0','25','50','75','100'), activecolor='black')
for lbl in duty1.labels: lbl.set_color('black')

def duty1_changed(label):
    send_pwm_cmd(f"D1_{label}")
duty1.on_clicked(duty1_changed)

# CH2 controls
fig.text(0.33, 0.17, "PWM CH2", color='white', fontsize=12, weight='bold')

ax_freq2 = plt.axes([0.33, 0.10, 0.12, 0.07], facecolor='white')
freq2 = RadioButtons(ax_freq2, ('500','1000','5000','10000','20000'),
                     activecolor='black')
for lbl in freq2.labels: lbl.set_color('black')

def freq2_changed(label):
    send_pwm_cmd(f"F2_{label}")
freq2.on_clicked(freq2_changed)

ax_duty2 = plt.axes([0.46, 0.10, 0.12, 0.07], facecolor='white')
duty2 = RadioButtons(ax_duty2, ('0','25','50','75','100'), activecolor='black')
for lbl in duty2.labels: lbl.set_color('black')

def duty2_changed(label):
    send_pwm_cmd(f"D2_{label}")
duty2.on_clicked(duty2_changed)

# CH3 controls
fig.text(0.62, 0.17, "PWM CH3", color='white', fontsize=12, weight='bold')

ax_freq3 = plt.axes([0.62, 0.10, 0.12, 0.07], facecolor='white')
freq3 = RadioButtons(ax_freq3, ('500','1000','5000','10000','20000'),
                     activecolor='black')
for lbl in freq3.labels: lbl.set_color('black')

def freq3_changed(label):
    send_pwm_cmd(f"F3_{label}")
freq3.on_clicked(freq3_changed)

ax_duty3 = plt.axes([0.75, 0.10, 0.12, 0.07], facecolor='white')
duty3 = RadioButtons(ax_duty3, ('0','25','50','75','100'), activecolor='black')
for lbl in duty3.labels: lbl.set_color('black')

def duty3_changed(label):
    send_pwm_cmd(f"D3_{label}")
duty3.on_clicked(duty3_changed)
...
def update(frame):
    try:
        if ser.in_waiting > 0:
            ...
            # PWM status messages: PWRn:freq,duty
            if decoded_line.startswith("PWR1:"):
                try:
                    payload = decoded_line.split("PWR1:")[1]
                    f, d = payload.split(',')
                    pwm_label_ch1.set_text(f"CH1 PWM: {f} Hz | {d} %")
                except:
                    pass
                return line1, line2, line3, line_xy

            if decoded_line.startswith("PWR2:"):
                try:
                    payload = decoded_line.split("PWR2:")[1]
                    f, d = payload.split(',')
                    pwm_label_ch2.set_text(f"CH2 PWM: {f} Hz | {d} %")
                except:
                    pass
                return line1, line2, line3, line_xy

            if decoded_line.startswith("PWR3:"):
                try:
                    payload = decoded_line.split("PWR3:")[1]
                    f, d = payload.split(',')
                    pwm_label_ch3.set_text(f"CH3 PWM: {f} Hz | {d} %")
                except:
                    pass
                return line1, line2, line3, line_xy
            ...
            if decoded_line.startswith("METRICS:"):
                ...
                ax_main.set_xlim(0, samples if samples > 0 else 50)
